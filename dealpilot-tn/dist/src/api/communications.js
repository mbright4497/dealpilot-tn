...
router.post('/send', async (req, res) => {
    try {
        ...
        if (!confirmed)
            return res.status(400).json({ error: 'confirm_required', message: 'Send must be explicitly confirmed.' });
        ...
        if (!to)
            return res.status(400).json({ error: 'missing_to', message: 'Recipient (to) is required.' });
        if (!subject)
            return res.status(400).json({ error: 'missing_subject', message: 'Subject is required.' });
        if (!body)
            return res.status(400).json({ error: 'missing_body', message: 'Body is required.' });
        // Log to console
        ...
        try {
            await supabaseClient_1.supabaseClient.from('communications_log').insert([{
