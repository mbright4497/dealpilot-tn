...
const compose_modals_1 = require("./compose-modals");
function CommunicationsPage() {
    // Compose modals included
    const [tab, setTab] = (0, react_1.useState)('messages');
    return (<div className="p-6 bg-[#061021] min-h-screen text-gray-100">
      <header className="mb-6">
        <h1 className="text-3xl font-bold">Communications</h1>
        <p className="text-sm text-gray-300">Manage all deal conversations</p>
      </header>
      <div className="mb-4 flex gap-2">
        {['messages', 'email', 'calls'].map(t => (<button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-t-lg ${tab === t ? 'bg-gray-800 text-orange-400' : 'bg-gray-700 text-gray-300'}`}>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-b-lg p-4 grid grid-cols-3 gap-4">
        ...
        {tab === 'calls' && (<div className="col-span-3 grid grid-cols-1 gap-3">
      ...
      <compose_modals_1.ComposeModals />
    </div>);
}
