...
const collectAllPartyEmails = (contacts) => {
    const emails = contacts
...
const formatToField = (emails, label) => {
    if (emails.length === 0)
        return `[Warning: ${label} email not on file]`;
    return emails.join(', ');
};
const buildDraftSubject = (kind, addr) => {
    if (kind === 'lender')
        return `Request: Loan status update for ${addr}`;
    if (kind === 'title')
        return `Request: Title update for ${addr}`;
    if (kind === 'closing')
        return `Reminder: Closing timeline for ${addr}`;
    return '';
};
const buildDraftBody = (kind, addr, buyer, contactName) => {
    if (kind === 'lender') {
...
function buildDraftTemplate(kind, transaction, contacts) {
    const addr = transaction.address || '';
    const buyer = transaction.client || '';
    const template = { subject: '', body: '', to: '' };
    if (kind === 'lender') {
