import type { FormDefinition, FormRule, DealContext } from '../types/forms';
...
export const FORM_RULES: FormRule[] = [
  {
    ...
    condition: (context: DealContext, formValues: any) => {
      if (!formValues || typeof formValues.appraised_value !== 'number') return false;
  ...
  },
  {
    ...
    condition: (context: DealContext, formValues: any) => {
      if (!formValues || typeof formValues.earnest_amount !== 'number') return false;
      return formValues.earnest_amount < (context.price ?? 0) * 0.01; // less than 1%
    }
  }
];

export default { FORM_DEFINITIONS, FORM_RULES };
