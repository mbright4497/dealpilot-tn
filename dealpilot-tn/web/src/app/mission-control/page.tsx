    ...
    <div>
      <div className="grid gap-1" style={{gridTemplateColumns:`repeat(${cols}, 1fr)`}}>
        {Array.from({length:rows*cols}).map((_,i)=>{
          ...
          return (
            <div key={i} className="aspect-square bg-[#071033] rounded border border-gray-800 flex flex-col items-center justify-center text-xs">
              {agent ? (<><span className={`w-4 h-4 rounded-full ${agent.color}`}></span><span className="mt-1">{agent.name}</span></>) : null}
            </div>
          )
        })}
      </div>
