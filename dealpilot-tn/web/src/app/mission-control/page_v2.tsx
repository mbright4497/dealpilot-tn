        ...
        <div className="mb-6 space-y-2">
          {team.map(a=>(
            <div key={a.id} className="flex items-center gap-2">
              ...
              <span className={`text-xs ${a.is_leader?'text-amber-400 font-bold':'text-gray-300'}`}>{a.name}</span>
              {a.is_leader&&<span className="text-[9px] text-amber-600">LEAD</span>}
            </div>
          ))}
        </div>
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-[#1e293b] border border-gray-700 rounded-xl p-6 w-full max-w-md shadow-2xl" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-4">
          <div className={`w-10 h-10 rounded-full ${agent.color} flex items-center justify-center font-bold text-black text-lg`}>{agent.name[0]}</div>
          <div>
            <div className="font-semibold">{agent.name} {agent.is_leader&&<span className="text-amber-400 text-xs ml-1">LEADER</span>}</div>
            <div className="text-xs text-gray-400">{agent.role}</div>
          </div>
          <StatusBadge status={agent.status}/>
        </div>
        {agent.current_task&&(
          <div className="mb-3 p-2 bg-gray-900 rounded text-xs text-gray-300">
            <span className="text-gray-500">Currently: </span>{agent.current_task}
          </div>
        )}
        {sent?(
          <div className="text-green-400 text-center py-4 font-medium">Command dispatched to OpenClaw</div>
        ):(
          <>
            <textarea value={cmd} onChange={e=>setCmd(e.target.value)}
              className="w-full bg-gray-900 text-white rounded px-3 py-2 text-sm resize-none h-24 border border-gray-700 focus:border-blue-500 outline-none"
              placeholder={`Tell ${agent.name} what to do...`}
              onKeyDown={e=>{if(e.key==='Enter'&&e.metaKey)send()}}/>
            <div className="flex gap-2 mt-3">
              <button onClick={send} disabled={sending||!cmd.trim()}
                className="flex-1 py-2 bg-blue-600 rounded hover:bg-blue-700 disabled:opacity-50 font-medium text-sm">
                {sending?'Dispatching...':'Tap on Shoulder'}
              </button>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Click any agent to tap them on the shoulder</p>
        </div>
            ...
            return(
              <div key={i}
                ...
                className={`aspect-square rounded flex flex-col items-center justify-center text-xs relative
                  ${isEven?'bg-[#0d1f4e]':'bg-[#0a1640]'}
                  ${agent?'cursor-pointer hover:ring-2 hover:ring-blue-500 transition-all':''}`}>
                {agent&&(
                  <>
        ...
        {team.map(a=>(
          <div key={a.id} onClick={()=>setSelected(a)}
            className={`p-3 rounded-lg border cursor-pointer transition-all hover:border-blue-500
              ${a.is_leader?'border-amber-600 bg-amber-950/20':'border-gray-800 bg-[#071033]'}`}>
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {activity.map(e=>(
              <div key={e.id} className="flex items-start gap-2 text-xs py-1 border-b border-gray-900">
                ...
                <span className={`px-1.5 rounded flex-shrink-0 ${
                  e.event_type==='command'?'bg-blue-900 text-blue-300':
                  e.event_type==='heartbeat'?'bg-gray-800 text-gray-400':
                  e.event_type==='result'?'bg-green-900 text-green-300':'bg-gray-800 text-gray-400'}`}>
                  {e.event_type}
                </span>
        ...
        {team.map(a=>(
          <div key={a.id} className={`p-5 rounded-xl border ${a.is_leader?'border-amber-600 bg-amber-950/10':'border-gray-800 bg-[#071033]'}`}>
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-12 h-12 rounded-full ${a.color} flex items-center justify-center font-bold text-black text-lg`}>
                {a.name[0]}{a.is_leader&&<span className="absolute text-[10px] -mt-6 ml-6">👑</span>}
              </div>
              <div>
                ...
                <div className="text-[10px] text-gray-600 mt-0.5">OpenClaw: {a.openclaw_id} · {a.model}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 mb-3">
              <StatusBadge status={a.status}/>
              {a.last_heartbeat&&<span className="text-[10px] text-gray-600">Last pulse: {new Date(a.last_heartbeat).toLocaleTimeString()}</span>}
            </div>
