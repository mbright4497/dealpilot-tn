...
export type TimelineEvent = { label:string, date:string|null, status:'completed'|'active'|'upcoming' }
export type Transaction = { id:number, address:string, client:string, type:'Buyer'|'Seller', status:'Active'|'Pending'|'Closed', binding:string, closing:string, contacts:Contact[], notes:string, current_state?:string, state_label?:string, binding_date?:string|null, closing_date?:string|null, inspection_end_date?:string|null, purchase_price?:number|null, earnest_money?:number|null, seller_names?:string|null, buyer_names?:string|null, financing_contingency_date?:string|null, special_stipulations?:string|null, contract_type?:string|null, timeline?:TimelineEvent[], issues?:any[], documents?:any[] }
...
const NAV_ITEMS = [ { id: 'dashboard', label: 'Dashboard' }, { id: 'transactions', label: 'Transactions' }, { id: 'forms', label: 'Forms Library' }, { id: 'deadlines', label: 'Deadlines' }, { id: 'tx-steps', label: 'TX Steps' }, { id: 'personality', label: 'Style' }, ]
...
export default function ChatPage() {
    ...
    fetch('/api/deal-state/all')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          setTransactions(data.map((d: any) => ({ id: d.id, address: d.address || '', client: d.client || '', type: d.type || 'Buyer', status: d.current_state === 'closed' ? 'Closed' : d.current_state === 'draft' ? 'Active' : 'Pending', binding: d.binding_date || '', closing: d.closing_date || '', contacts: [], notes: '', current_state: d.current_state || 'draft', state_label: d.state_label || 'Draft', binding_date: d.binding_date, closing_date: d.closing_date, inspection_end_date: d.inspection_end_date, purchase_price: d.purchase_price, earnest_money: d.earnest_money, seller_names: d.seller_names, buyer_names: d.buyer_names, financing_contingency_date: d.financing_contingency_date, special_stipulations: d.special_stipulations, contract_type: d.contract_type, timeline: d.timeline || [], issues: d.issues || [], documents: d.documents || [], })))
        }
      })
  ...
  async function addTransaction(tx: any) {
    try {
      const res = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({ address: tx.address || '', client: tx.client || '', type: tx.type || 'Buyer', status: tx.status || 'Active', binding: tx.binding || '', closing: tx.closing || '', notes: tx.notes || '', contacts: JSON.stringify(tx.contacts || []), purchase_price: tx.purchase_price || null, earnest_money: tx.earnest_money || null, seller_names: tx.seller_names || null, buyer_names: tx.buyer_names || null, inspection_end_date: tx.inspection_end_date || null, financing_contingency_date: tx.financing_contingency_date || null, special_stipulations: tx.special_stipulations || null, contract_type: tx.contract_type || null, timeline: JSON.stringify(tx.timeline || []), issues: JSON.stringify(tx.issues || []), documents: JSON.stringify(tx.documents || []), }),
      })
      const newTx = await res.json()
      if (newTx && newTx.id) {
        setTransactions(prev => [...prev, { ...newTx, contacts: typeof newTx.contacts === 'string' ? JSON.parse(newTx.contacts) : (newTx.contacts || []), timeline: typeof newTx.timeline === 'string' ? JSON.parse(newTx.timeline) : (newTx.timeline || []), issues: typeof newTx.issues === 'string' ? JSON.parse(newTx.issues) : (newTx.issues || []), documents: typeof newTx.documents === 'string' ? JSON.parse(newTx.documents) : (newTx.documents || []), }])
      }
    } catch (err) {
  ...
  function openDeal(txId: number) { setSelectedTxId(txId); setView('deal') }
  function openChecklist(txId: number) { setSelectedTxId(txId); setView('deal') }

  async function deleteTransaction(txId: number) { try { await fetch('/api/transactions', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: txId }), }) setTransactions(prev => prev.filter(t => t.id !== txId)) } catch (err) { console.error('Failed to delete transaction:', err) } }

  function updateTransactionContacts(txId: number, contacts: any[]) { setTransactions(prev => prev.map(t => t.id === txId ? {...t, contacts} : t)) }
  function handleNavigate(dest: string) { if (dest === 'transactions') { setSelectedTxId(null); setView('transactions') } else if (dest === 'forms') { setView('forms') } else if (dest === 'deadlines') { setView('deadlines') } else if (dest === 'checklist') { setView('transactions') } else if (dest === 'ai') { setChatOpen(true) } else { setView(dest) } }
  ...
  return (
    <div className="flex h-screen bg-dp-bg-dark">
      <aside className="w-60 bg-dp-sidebar flex flex-col border-r border-gray-800">
        ...
        <nav className="flex-1 px-3 py-2 space-y-1">{NAV_ITEMS.map(item => (
            <button key={item.id} onClick={() => { setView(item.id); setSelectedTxId(null) }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${ view === item.id ? 'bg-gray-800 text-orange-400' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }`}>
              <NavIcon name={item.id} />
              {item.label}
            </button>))}
          <button onClick={() => setChatOpen(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-all">
            <NavIcon name="ai" />
            AI Assistant
          </button>
        </nav>
